...
public:
	...
	shNginTexture();
	shNginTexture(const char *file, int alg = NGIN_NEAREST);		// Converts textures to RGBA by default
	shNginTexture(const unsigned char *file, int alg = NGIN_NEAREST);
	shNginTexture(const unsigned char *img, unsigned int w, unsigned int h, int m = NGIN_BGR, int alg = NGIN_NEAREST);
	...
	void shNginTLoad(const char *file, int alg = NGIN_NEAREST);
	void shNginTLoad(const unsigned char *file, int alg = NGIN_NEAREST);
	void shNginTLoad(const unsigned char *img, unsigned int w, unsigned int h,
			int m = NGIN_BGR, int alg = NGIN_NEAREST);
